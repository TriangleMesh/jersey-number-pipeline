...
#
# Modified for sports-specific jersey augmentation support.
...
class SceneTextDataModule(pl.LightningDataModule):
    ...
    @staticmethod
    def get_transform(img_size: Tuple[int], augment: bool = False, rotation: int = 0, jersey_aug: bool = False):
        transforms = []

        if augment:
            if jersey_aug:
                from .augment import SportsJerseyAugment
                transforms.append(SportsJerseyAugment())
            else:
    ...
    def train_dataset(self):
        if self._train_dataset is None:
            transform = self.get_transform(self.img_size, self.augment, jersey_aug=self.jersey_aug)
